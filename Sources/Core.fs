...
module Core =
  ...
  and Application () =
    ...
    member self.Init () =
      ...
      options.BackgroundColor <- Some (float 0xFFFFFF)
      options.Resolution <- Some 1.
      options.Antialias <- Some true
  ...
  module Default =
    ...
    let Switch =
      let activeTextStyle =
        ...
        style.FontFamilly <- Some "Arial"
        style.FontSize <- Some 14.
        style.Fill <- Some (U2.Case2 (float TURQUOISE))
        style
      ...
      let inactiveTextStyle =
        ...
        style.FontFamilly <- Some "Arial"
        style.FontSize <- Some 14.
        style.Fill <- Some (U2.Case2 (float CLOUDS))
        style
